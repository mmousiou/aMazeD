...
Turtle.init = function() {
    ...
    //Εμφάνιση επιλεγμένων μπλοκ στο workspace
    ...
    var defaultXml7 =
        ...
        '<field name="VALUE">100</field>' +
        '<next>' +
        '<block type="turtle_turn_internal"><field name="DIR">turnLeft</field>' +
        '<field name="VALUE">90</field>' +
        '<next>' +
        '<block type="turtle_move_internal">' +
        '<field name="VALUE">100</field>' +
        ...
        '<block type="turtle_move_internal">' +
        '<field name="VALUE">100</field>' +
        '</block>' + // τέλος 7ου block 
        '</next>' +
        '</block>' + // τέλος 6ου block 
        '</next>' +
        '</block>' + // τέλος 5ου block 
        '</next>' +
        '</block>' + // τέλος 4ου block 
        '</next>' +
        '</block>' + // τέλος 3ου block 
        '</next>' +
        '</block>' + // τέλος 2ου block 
        '</next>' +
    ...
    var defaultXml10 =
        ...
        '</block>' + // τέλος 3oυ block     
        '</next>' +
        '</block>' + // τέλος internal repeat
        '</statement>' +
        '</block>' + // τέλος external repeat
        '</xml>';


    if (BlocklyGames.LEVEL == 7) {
        BlocklyInterface.loadBlocks(defaultXml7, false);
    }

    if (BlocklyGames.LEVEL == 8) {
        BlocklyInterface.loadBlocks(defaultXml8, false);
    }

    if (BlocklyGames.LEVEL == 9) {
        BlocklyInterface.loadBlocks(defaultXml9, false);
    }

    if (BlocklyGames.LEVEL == 10) {
        BlocklyInterface.loadBlocks(defaultXml10, false);
    }
    ...
    if (BlocklyGames.LEVEL == 7) {
        ...
        setTimeout(Turtle.showCategoryHelp, 1000);
